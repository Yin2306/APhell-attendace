...
def process_single_account(acc, otp):
    with sync_playwright() as p:
        browser = None
        try:
            # Add more aggressive launch parameters
            ...
            # --- Interception logic: only allow necessary login requests, block images, fonts, tracking scripts ---
            def block_aggressively(route):
                resource_type = route.request.resource_type
                if resource_type in ["image", "media", "font", "stylesheet"]:
                    route.abort()
                elif "browser.events.data.microsoft.com" in route.request.url:  # block Microsoft telemetry
                    route.abort()
                else:
                    route.continue_()
            ...
            # Shorten wait time since resources are blocked (page will never reach full "networkidle")
            ...
            # Post-login confirmation button
            try:
                page.wait_for_selector('#idSIButton9', timeout=4000)
                page.click('#idSIButton9')
            except:
                pass

            # Listen for token (exit immediately once captured)
            for _ in range(100):
                if token_container:
                    ...
                    if result.get("data") and result["data"].get("updateAttendance"):
                        print(f"✅ {acc['name']}: Success")
                        return f"✅ {acc['name']}: Success"
                    else:
                        error_detail = result.get("errors", [{}])[0].get("message", "Unknown API error")
                        print(f"❌ {acc['name']} ERROR: {error_detail}") 
            ...
            if browser: browser.close()
            print(f"❌ {acc['name']} SYSTEM ERROR: {str(e)}")
            return f"❌ {acc['name']}: Failed"
...
if __name__ == "__main__":
    ...
    num_workers = 3  # Most stable setting
    
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        final_reports = list(executor.map(lambda acc: process_single_account(acc, otp), accounts))

    end_time = time.time()
    report_text = f"🏁 Attendance task finished | Time taken: {int(end_time - start_time)}s\n\n" + "\n".join(final_reports)
